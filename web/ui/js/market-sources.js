...
function sourceButtonsHtml(s) {
  ...
  const busy = (action, label) =>
    ...
    `${inflight ? " disabled" : ""}` +
    `${inflight === action ? ` class="market-source-busy">${label}…</button>` : `>${label}</button>`}`;
  let html = "";
...
function renderSourceDetail(provider, sources) {
  ...
  const staleHint = stale
    ? `<tr><td class="market-source-prop"></td>` +
      `<td class="market-source-stale">Feed task exited — status may be stale` +
  ...
  el.innerHTML = `
    <table class="data-table">
      <tr><td class="market-source-prop-first">Provider</td><td>${escDash(s.provider)}</td></tr>
      <tr><td class="market-source-prop">App Credentials</td><td>${creds}</td></tr>
      <tr><td class="market-source-prop">Daily Login</td><td>${dailyLogin}</td></tr>
      <tr><td class="market-source-prop">Feed State</td><td><span class="${stateCls}">${stateFriendly}</span></td></tr>
      <tr><td class="market-source-prop">Task Running</td><td>${s.task_running == null ? "—" : (s.task_running ? "Yes" : "No")}</td></tr>
      ${staleHint}
      ${authHint}
      <tr><td class="market-source-prop">Mode</td><td>${escDash(s.mode)}</td></tr>
      <tr><td class="market-source-prop">Instruments</td><td>${subd}</td></tr>
      <tr><td class="market-source-prop">Connect Attempts</td><td>${s.connect_attempts ?? 0}</td></tr>
      <tr><td class="market-source-prop">Reconnects</td><td>${s.reconnect_count ?? 0}${s.reconnecting ? " (reconnecting now)" : ""}</td></tr>
      <tr><td class="market-source-prop">Frames Received</td><td>${s.frames_received ?? 0}</td></tr>
      <tr><td class="market-source-prop">Malformed Frames</td><td>${s.malformed_frames ?? 0}</td></tr>
      <tr><td class="market-source-prop">Last Connected</td><td>${escDash(s.last_connected_at)}</td></tr>
      <tr><td class="market-source-prop">Last Message</td><td>${escDash(s.last_message_at)}</td></tr>
      <tr><td class="market-source-prop">Last Error</td><td>${escDash(s.last_error)}</td></tr>
      <tr><td class="market-source-prop">Last Exit</td><td>${escDash(s.last_exit_reason)}${s.last_exit_at ? ` at ${escDash(s.last_exit_at)}` : ""}</td></tr>
      <tr><td class="market-source-prop">Stop Reason</td><td>${stopReasonLabel(s.stop_reason)}</td></tr>
      ${s.not_ready_reason ? `<tr><td class="market-source-prop">Not Ready</td><td>${escDash(s.not_ready_reason)}</td></tr>` : ""}
      ${renderTransitions(s.recent_transitions)}
      <tr><td class="market-source-prop">Controls</td><td>${sourceButtonsHtml(s)}</td></tr>
    </table>`;
}
...
function renderTransitions(transitions) {
  ...
  const rows = transitions.slice(-8).reverse().map((t) => {
    ...
    return `<div class="market-source-meta">` +
      `${escDash(at)} &nbsp; ${escDash(t.from)} → ${escDash(t.to)}${reason}</div>`;
  }).join("");
  return `<tr><td class="market-source-transitions-label">Recent Transitions</td>` +
    `<td>${rows}</td></tr>`;
}
