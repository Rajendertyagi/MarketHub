...
Covers removal of same-instrument/same-chain/mixed-source restrictions:
  * BT1 multi-quote ANY group (different instruments, one triggers)
  * BT2 multi-chain analytics group
  * BT3 mixed quote + analytics group
  * BT4 cross-instrument crossing detection
  * BT5 nested mixed group (ALL of ANY with mixed sources)
...
# ---------------------------------------------------------------------------
# BT1: Multi-quote ANY group
# ---------------------------------------------------------------------------

async def test_bt1_multi_quote_any(runner: R) -> None:
    """BT1: Multi-instrument ANY — one true is enough to fire."""
    store, tmp = _mk_store()
    try:
        ...
        runner.assert_eq("BT1-rel-fired", len(fired), 1)
        ...
        runner.assert_eq("BT1-infy-fired", len(fired), 1)
    finally:
        tmp.cleanup()
...
async def test_bt2_multi_chain_analytics(runner: R) -> None:
    """BT2: Analytics group with different expiries accepted."""
    ...
    try:
        ...
        runner.assert_eq("BT2-deps-count", len(deps), 2)
        has_pcr = any("2026-09-25" in d for d in deps)
        has_iv = any("2026-10-30" in d for d in deps)
        runner.assert_true("BT2-has-pcr-dep", has_pcr)
        runner.assert_true("BT2-has-iv-dep", has_iv)
    finally:
        tmp.cleanup()
...
async def test_bt3_mixed_quote_analytics(runner: R) -> None:
    """BT3: Mixed quote and analytics leaves in same group accepted."""
    from unittest.mock import MagicMock
    ...
    try:
        ...
        runner.assert_true("BT3-has-quote-dep", has_quote)
        runner.assert_true("BT3-has-analytics-dep", has_analytics)
    finally:
        tmp.cleanup()
...
async def test_bt4_cross_instrument(runner: R) -> None:
    """BT4: Crossing operators across different instruments."""
    store, tmp = _mk_store()
    try:
        _create_v2(store, [
        ...
        fired = await engine.evaluate(_FakeQuote(101, token="2885"))
        runner.assert_eq("BT4-rel-crossed", len(fired), 0)
        ...
        runner.assert_in("BT4-infy-crossed", len(fired), [0, 1])
    finally:
        tmp.cleanup()


# ---------------------------------------------------------------------------
# BT5: Nested mixed group
# ---------------------------------------------------------------------------

async def test_bt5_nested_mixed(runner: R) -> None:
    """BT5: Nested group: ALL [ ANY(quote1, quote2), analytics1 ]."""
    from unittest.mock import MagicMock
    ...
    try:
        ...
        aid = store.create_condition_alert(
            ...
            trigger_mode="repeat", condition_json=normalized)
        runner.assert_true("BT5-created", bool(aid))

        from core.persistence.modules.condition_alerts import validate_condition_tree
        alert = store.get_condition_alert(aid)
        cond = validate_condition_tree(alert["condition"])
        deps = ConditionAlertEngine._get_dependency_keys(cond)
        runner.assert_eq("BT5-deps-count", len(deps), 3)
        quote_deps = [d for d in deps if d.startswith("quote:")]
        analytics_deps = [d for d in deps if d.startswith("analytics:")]
        runner.assert_eq("BT5-quote-deps", len(quote_deps), 2)
        runner.assert_eq("BT5-analytics-deps", len(analytics_deps), 1)
    finally:
        tmp.cleanup()
...
async def main() -> bool:
    runner = R()
    await test_bt1_multi_quote_any(runner)
    await test_bt2_multi_chain_analytics(runner)
    await test_bt3_mixed_quote_analytics(runner)
    await test_bt4_cross_instrument(runner)
    await test_bt5_nested_mixed(runner)
