...
class _DummyBus:
    """No-op bus that accepts publish calls."""
    def __init__(self):
...
async def run():
    ...
    for n_subscribers in [1, 10, 100]:
        ...
        times = []
        for idx in range(WARMUP + MEASURE):
        ...
        rows.append({
            ...
            "total_p50_ms": round(_percentile(times, 50), 4),
            "total_p95_ms": round(_percentile(times, 95), 4),
            "total_p99_ms": round(_percentile(times, 99), 4),
            "iterations": MEASURE,
            "bus_publish_calls": bus.publish_count,
        })
