...
describe("buildSectorHeatmapOption", () => {
  ...
  it("builds a treemap with one group per sector and preserves Unclassified", () => {
    ...
    const groups = opt.series[0]!.data;
    ...
    const leaf = (uncGroup.children[0] as { itemStyle: { color: string } });
...
describe("buildMarketMapOption", () => {
  ...
  function leaves(opt: EChartsOption) {
    const series = (opt as unknown as { series: Array<{ data: Array<{ children?: unknown[] }> }> }).series;
    return series[0]!.data.flatMap((g) => (g.children ?? []) as Array<{ value: number[]; itemStyle: { color: string }; _meta: { symbol: string } }>);
  }

  it("uses equal area (weight 1) in equal mode", () => {
    const opt = buildMarketMapOption(snap.sectors, "equal");
    leaves(opt).forEach((l) => expect(l.value[0]).toBe(1));
  });
  ...
  it("uses a bounded volume weight in volume mode", () => {
    ...
    expect(rel.value[0]).toBeGreaterThan(1);
    expect(rel.value[0]).toBeLessThanOrEqual(3);
  });
