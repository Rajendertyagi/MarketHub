...
export function NewsSourcesPane({ sources, filters, onFilterChange, onManageClick }: Props) {
  const selectedSource = filters.source_ids?.[0] ?? "";
  const set = (patch: Partial<NewsFilters>) => onFilterChange({ ...filters, ...patch });
  ...
  return (
    <aside className="news-pane news-sources-pane">
      <div className="news-pane-header">
        <h2>Sources</h2>
        <button type="button" className="btn btn-compact" onClick={onManageClick}>
          Manage
        </button>
      </div>
        ...
        {enabled.map((s) => (
          <li key={s.source_id}>
            ...
            >
              ...
              <span className={`chip chip-muted news-source-type`}>{s.source_type}</span>
            </button>
          </li>
        ))}
        {enabled.length === 0 ? (
