        ...
        {consumers.map((c) => (
          <div className="ai-consumer-card" key={c.consumer_id}>
            ...
            <div className="ai-consumer-meta">
              <div>
                Pending:{" "}
                <strong className={c.pending_count > 0 ? "ai-consumer-pending" : ""}>
                  {c.pending_count}
                </strong>
              </div>
              <div>
                Unacked: <strong>{c.unacknowledged_count}</strong>
              </div>
              <div>
                Last trigger: {c.last_triggered ? timeAgo(c.last_triggered.trigger_time) : "—"}
              </div>
              <div>
                Checkpoint: {c.last_checkpoint ? `#${c.last_checkpoint.last_sequence}` : "—"}
              </div>
            </div>
            <div className="ai-consumer-foot muted">id {shortId(c.consumer_id)}</div>
          </div>
        ))}
      </div>
    </div>
