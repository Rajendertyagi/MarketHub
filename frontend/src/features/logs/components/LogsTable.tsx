...
export const LogsTable = forwardRef<HTMLDivElement, { records: LogRecord[] }>(function LogsTable(
  { records },
  ref,
) {
  if (!records.length) {
    ...
    <div className="logs-container" ref={ref}>
      <table className="table logs-table table-compact">
        ...
        <tbody>
          {records.map((r) => (
            <tr
              key={`${r.ts}-${r.logger}-${r.message}`}
              className={`logs-row logs-level-${r.level.toLowerCase()}`}
            >
