...
export function NotificationsPanel({ notifications }: { notifications: AlertNotification[] }) {
  ...
  return (
    <div className="panel">
      ...
      <ul className="notifications-list">
        {notifications.map((n) => (
          <li key={`${n.alert_id}-${n.ts}`} className="hint err notification-item">
            {n.tradingsymbol}: {formatCondition(n.field, n.operator, n.threshold)} (now{" "}
            {n.value ?? "—"})
          </li>
        ))}
      </ul>
